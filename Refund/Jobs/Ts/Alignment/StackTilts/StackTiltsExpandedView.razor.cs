...
public partial class StackTiltsExpandedView : IAsyncDisposable
{
    #region Dependencies
    ...
    [Inject] private ILogger<StackTiltsExpandedView> Logger { get; set; }
    ...
    private ReadOnlyStackTilts _job;
    ...
    {
        if (job is ReadOnlyStackTilts importAlignments)
        {
            ...
            _micrographs = _job.PortsIn[StackTilts.PortInDataSetTs].GetSingleResource<DataSetTs>().Micrographs;
    ...
    private async Task LoadData()
    {
        ...
        {
            Logger.LogError(ex, "Error loading stack tilts data for job {JobId}", _job?.Id);
        }
    }
    ...
    private async Task SelectTsItem(int index, bool scrollToItem)
    {
        ...
        var tsSet = (TiltSeriesSet)_job.PortsOut[StackTilts.PortOutDataSetTs].GetResource();
