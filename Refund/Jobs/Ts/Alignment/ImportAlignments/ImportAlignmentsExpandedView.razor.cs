...
public partial class ImportAlignmentsExpandedView : IAsyncDisposable
{
    #region Dependencies
    ...
    [Inject] private ILogger<ImportAlignmentsExpandedView> Logger { get; set; }
    ...
    private ReadOnlyImportAlignments _job;
    ...
    {
        if (job is ReadOnlyImportAlignments importAlignments)
        {
            _job = importAlignments;
            _processedTsItems = null;
            _allTsThumbnails = [];
            _selectedTsThumbnail = null;
            _selectedTsItem = null;

            _micrographs = _job.PortsIn[ImportAlignments.PortInTs].GetSingleResource<TiltSeriesSet>().DataSet.Micrographs;
    ...
    private async Task LoadData()
    {
        ...
        {
            Logger.LogError(ex, "Error loading import alignments data for job {JobId}", _job?.Id);
        }
    }
    ...
    private async Task SelectTsItem(int index, bool scrollToItem)
    {
        ...
        var tsSet = _job.PortsIn[ImportAlignments.PortInTs].GetSingleResource<TiltSeriesSet>();
