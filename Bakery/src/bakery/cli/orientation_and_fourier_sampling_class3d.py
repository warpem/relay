...
from ..spherical_hexbin.utils import (
    ...
    xyz_to_cell, rottilt_to_rotation_matrix, symmetrize
)

IMAGE_SIDELENGTH = 1024


@cli.command(no_args_is_help=True,
             help="hexbin plots of particle orientations and Fourier sampling distributions from RELION Class3D results")
def orientation_and_fourier_sampling_class3d(
    ...
    Output filenames will be as specified with a `_class{class_number:03d)` suffix
    e.g.
     - my_orientations.png -> my_orientations_class003.png
     - my_fourier_sampling.png -> my_fourier_sampling_class003.png
    """
    # read RELION star file containing Class3D results and group by class
    star = starfile.read(particle_star_file, always_dict=True)
    df = star['particles']
    grouped = df.groupby('rlnClassNumber')

    for class_number in range(1, n_classes + 1):
        try:
            process_single_class(
                df=grouped.get_group(class_number),
                class_number=class_number,
                grid_resolution=grid_resolution,
                symmetry=symmetry,
                colormap=colormap,
                orientation_file=output_orientation_file,
                fourier_sampling_file=output_fourier_sampling_file
            )
        except KeyError:
...
def process_single_class(
    ...
    fourier_sampling_file: Path,
):
    # grab rot and tilt angles from file
    rottilt = df[['rlnAngleRot', 'rlnAngleTilt']].to_numpy()
    rot, tilt = einops.rearrange(rottilt, 'b rt -> rt b')

    # preprocess angles and grab oriented z-vectors
    ...
    # get h3 indices for each vector at a given h3 resolution and count them
    ...
    rot_grid, tilt_grid = generate_rot_grid(IMAGE_SIDELENGTH), generate_tilt_grid(IMAGE_SIDELENGTH)
    ...
    if orientation_file is not None:
        orientation_file = orientation_file.with_stem(orientation_file.stem + suffix)
        ...
        plot_orientation_distribution(
            image=image,
            colormap=colormap,
            output_file=orientation_file
        )

    # fourier sampling distribution
    if fourier_sampling_file is not None:
        fourier_sampling_file = fourier_sampling_file.with_stem(fourier_sampling_file.stem + suffix)
        ...
        plot_fourier_sampling_distribution(
            image=image,
            colormap=colormap,
            output_file=fourier_sampling_file
        )
...
def generate_empty_plots(
    ...
    suffix = f'_class{class_number:03d}'
    empty_image = np.zeros((IMAGE_SIDELENGTH, IMAGE_SIDELENGTH))
